...
const recordSM = defineState(({ computed }) => {
            const accountRecord = stateManagerRecord({recordId: '001xx000003GYiCAAW', optionalFields: [`${ACCOUNT.objectApiName}.${NAME_FIELD.fieldApiName}`]}, {}); // Account.Name
            const customObjectRecord = stateManagerRecord({recordId: 'a00xx000000bnfRAAQ', optionalFields: [`${CUSTOM_OBJECT.objectApiName}.${CUSTOM_FIELD.fieldApiName}`]}, {}); // CustomObject__c.CustomField__c
    return {
        accountRecord,
        customObjectRecord
    };
