...
// Define the state manager. A recordId and objectApiName (both strings) can be
// specified when the state manager is created. These values can also be set/changed
// later using the state manager's setRecordId & setObjectApiName actions.
export default defineState(({ atom, computed, setAtom }, recordId, objectApiName) => {
    // This atom captures the current configuration of the state manager instance.
